...
### 공통 / 환경
- [ ] **CHATGPT_API_KEY 미설정 시 AI 기능 전부 500 에러** — 챗봇, 논문 주석, 힌트 챗봇 모두 `openai.AsyncOpenAI(api_key="")` 상태. `.env`에 키 설정 필요 (설정 안 하면 전부 에러)
- [ ] **DB 초기 데이터 없음** — Course/Lecture 시드 데이터가 없어서 커리큘럼·렉쳐 탭이 완전히 빈 상태. alembic seed 또는 API로 데이터 삽입 필요

### 홈
- [ ] **학기 진도 현황 하드코딩** — `GET /api/v1/curriculum/`의 실제 `progress_pct` 값을 쓰지 않고 42%/18%/5% 고정값 사용. API fetch 후 동적 렌더링으로 교체
- [ ] **사이드바 과목 % 하드코딩** — 좌측 사이드바 `<span class="sb-pct">42%</span>` 등이 전부 고정값. API 데이터로 교체
- [ ] **사이드바 과목 클릭 → 해당 과목 강의 페이지로 이동 안 됨** — `selectSubject()` 함수가 강조만 하고 `gotoLecture(courseId)` 호출 없음
- [ ] **프로필 드롭다운 수치 하드코딩** — 이수 학점, 전체 진도 23%, 최근 활동, AI 테스트 점수 전부 고정값. API로 실제 진도 가져와야 함
- [ ] **상단 "학사 포털", "진도 현황" 링크 클릭 무반응** — `onclick` 없음

### 렉쳐 노트
- [ ] **렉쳐 노트 내용 전부 비어있음** — DB에 `LectureNote` 데이터 없어서 모든 강의가 placeholder 표시. 최소한 선형대수 몇 강 분이라도 수동 입력 필요
- [ ] **YouTube 영상 전부 "동영상 없음"** — 크롤러 실행 전까지 `youtube_url = null`. YouTube 크롤러 1회 실행 후 해결됨

### 내 노트
- [ ] **노트 삭제 버튼 없음** — `DELETE /api/v1/notes/{id}` API는 있지만 UI가 없음. 노트 목록 아이템에 삭제 버튼 추가 필요

### 논문 읽기
- [ ] **논문 추가 UI 없음** — 사용자가 새 논문을 추가하는 방법이 없음. arXiv ID 입력 → seed 방식으로 추가하는 폼 필요
- [ ] **논문 검색 없음** — 논문이 많아지면 목록 스크롤만으로는 찾기 어려움. 제목/저자 검색 input 필요

### 테크 블로그
- [ ] **소스 필터 클릭 → 피드 필터링 안 됨** — `filterBlog(type)` 함수가 `.src-item` 스타일만 토글하고 `BLOG_POSTS` 실제 필터링 없음
- [ ] **상단 카테고리 탭 (LLM/비전/인프라/추천시스템) 클릭해도 아무것도 안 됨** — `setBlogTab` 함수가 버튼 스타일만 변경, 실제 필터 로직 없음
- [ ] **피드 검색 input 동작 안 함** — `<input class="feed-search">` 에 `oninput` 이벤트 없음
- [ ] **아티클 뷰어에 "원문 보기" 링크 없음** — 피드 아이템의 `url` 필드가 있지만 아티클 뷰어에 원문 링크 버튼이 없음

### 지식 그래프
- [ ] **노드 클릭 → 연결된 노트 없으면 아무 반응 없음** — 연결 노트를 못 찾으면 조용히 실패. "관련 노트 없음" 피드백 또는 새 노트 작성 유도 필요
- [ ] **그래프 데이터 항상 fallback** — DB에 노트 저장 전까지 GPT 키워드 추출이 안 돌아서 `GraphNode`가 0개 → fallback 하드코딩 데이터만 표시

### AI 챗봇
- [ ] **컨텍스트 바 "선형대수학 Lecture 03" 하드코딩** — 현재 보고 있는 실제 강의 상태를 반영하지 않음
- [ ] **취약 개념 칩 하드코딩** — "고유값", "행렬식", "LU 분해", "SVD" 고정값. 실제 진도·테스트 기록 기반으로 동적 생성 필요
