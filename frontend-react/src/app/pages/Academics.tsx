...
export function Academics() {
  const [activeId, setActiveId] = useState("greeting");
  ...
  return (
    <>
      ...
      {/* 본문 레이아웃 */}
      <div className="max-w-7xl mx-auto px-6 py-16 flex gap-12 items-start">
        ...
        <aside className="hidden md:flex flex-col w-48 flex-shrink-0 sticky top-[90px]">
          ...
          <nav className="flex flex-col gap-1">
            {TABS.map(({ id, label }) => (
              ...
              >
                {activeId === id && <span className="mr-1.5 text-primary">›</span>}
                {label}
              </button>
            ))}
          </nav>
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-24">
          ...
          <section id="greeting" className="scroll-mt-24">
            ...
            <div className="grid md:grid-cols-[1fr_180px] gap-12 items-start">
              ...
              <div className="flex flex-col items-center gap-3">
                <div className="w-36 h-36 rounded-full bg-muted border flex items-center justify-center text-5xl">🎓</div>
                <div className="text-center">
                  <div className="font-bold text-sm">설립자</div>
                  <div className="text-xs text-muted-foreground mt-1 leading-relaxed">
                    Founder & Rector<br />Self-Taught University<br />Est. 2025
                  </div>
                </div>
              </div>
            </div>
          ...
          <section id="majors" className="scroll-mt-24">
            ...
            <p className="text-muted-foreground mb-10">STU가 운영 중인 자기주도형 학위 과정입니다.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {MAJORS.map((m) => (
                <div key={m.code} className="rounded-xl border bg-muted/30 p-6 flex flex-col gap-4 hover:border-primary/40 transition-colors">
                  <div>
          ...
          <section id="vision" className="scroll-mt-24">
            ...
            <div className="space-y-4">
              {VISION.map(({ num, title, desc }) => (
                <div key={num} className="flex gap-6 p-6 rounded-xl border bg-muted/30 hover:border-primary/40 transition-colors">
